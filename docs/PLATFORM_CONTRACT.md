...
| Workspace files | Kubernetes PVC and CSI `StorageClass` | Shared RWO local-path claim | Shared RWX claim, verified capacity, snapshots/backup, and restore testing |
...
| Elastic Runtime capacity | Kubernetes node labels, taints, and `RuntimeClass` | A pre-created local worker | A node-pool controller that verifies Node Ready, CNI, runtime handler, and workload schedulability |
...
unlabelled single-node Kubernetes cluster. A production node-pool adapter must set
...
PVC/object-store contract, not from a worker's local filesystem.
...
`subPath` into each Runtime. This works with RWO on the single-node local profile
and requires RWX for multi-node production. Requested PVC capacity is not proof of
