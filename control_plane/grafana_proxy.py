...
1. **Only an administrator gets through.**  Enforced by the caller before it
   reaches ``forward``; the Control Plane metrics behind these panels carry no
   tenant label, and that is deliberate (see ``control_plane/core.py`` at
   ``METRICS`` and ``observability/README.md``): the label would grow
   without bound with the tenant count, and ``/metrics`` is unauthenticated.
   The consequence is that every panel is cross-tenant data - an operator
...
DEFAULT_DASHBOARD_UID = "sandbox-control-plane"
...
#tests/test_grafana_embed.py asserts every id below is still in the shipped dashboard JSON.
PANELS: tuple[tuple[int, str], ...] = (
    (1, 'Live runtimes'),
    (2, 'Registered workspaces'),
    (3, 'Creations in flight'),
    (4, 'Object-store queue depth'),
    (10, 'Runtime creation failures by reason'),
    (11, 'Admission refusals by gate'),
    (12, 'Cold start latency'),
    (13, 'Store errors and dropped audit rows'),
    (14, 'Reaper actions by kind'),
    (15, 'Authenticated requests by credential kind'),
)
