...
noticed the prefix reappearing in a manifest, a Dockerfile, a chart or a
document.
...
* a scan face that is too narrow.  Here it is every git-tracked path, and
  :class:`ScanFaceTests` asserts both that the odd corners are in it and that
  files are really being read.
* a matcher that cannot fire - this repository has shipped that shape already, a
  banned variable name that matched nothing from the day it was written.
  :class:`MatcherTests` pins the matcher in both directions, because the prefix
  is a substring of ``minimum``, ``minio``, ``administrator`` and
  ``deterministic`` - all legitimate, all present here, so a bare substring test
...
class ScanFaceTests(unittest.TestCase):
    ...
    def test_scan_face_actually_reads_files(self) -> None:
        ...
        self.assertGreater(len(scanned), 200)
