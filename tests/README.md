...
The suite needs the standard library plus the `test` extra in `pyproject.toml`
(PyYAML, and boto3 so that `control_plane.core` imports). Modules that read
...
| `sandbox_client.py` | `test_sandbox_client.py` | Object owner defaults and validation; `normalize_workspace_path` (the one client-side normalisation point every `file_*` tool goes through) makes `/workspace`-rooted paths relative and refuses paths outside it, home paths and empty strings, each with its own message; fail-closed without a token, resolve/lookup never create runtimes, command quoting, named runtime reuse. |
